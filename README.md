...
* **Artifact Recovery UX**: GCS に保存された `Recipe JSON` と `WAV` を履歴画面から再参照し、署名付き URL 経由で安全に再生。
...
| **History** | GCS 上の生成履歴を一覧・詳細表示し、音声プレビューを提供 | Job ID / Audio Preview・Recipe JSON |
...
5. **History**: 認証済みユーザーは `/web/history` から生成済みジョブを一覧し、詳細画面で音声再生・レシピ確認・JSONコピーが可能。
...
6. **Repository 層 (Data Access)**
   - GCS 上の成果物一覧・レシピ取得を担当し、履歴画面と詳細画面へ永続化済みデータを供給。
...
│       ├── history.html            # 生成履歴一覧と簡易プレビュー
│       ├── music_view.html         # 楽曲詳細・音声再生・Recipe JSON 表示
│       └── layout.html             # 共通レイアウト
...
│   │   ├── storage_cleanup.go      # 失敗時の部分成果物削除
│   │   ├── slack.go                # Slack 通知
│   │   └── *_test.go               # アダプタ単体テスト
...
│   ├── repository/
│   │   └── music_repository.go     # GCS 上の履歴一覧・レシピ取得
│   └── server/                     # HTTP サーバーとルーティング
│       ├── handlers/
│       │   ├── handler.go          # Web UI の表示とテンプレート描画
│       │   ├── task_factory.go     # フォーム入力から Task を生成
│       │   ├── history_handler.go  # 履歴一覧・詳細・音声リダイレクト
│       │   ├── task_handler.go     # ジョブ投入
...
| **履歴参照** | **GCS Repository** | 保存済み Recipe JSON の一覧・詳細取得 |
